...
/**@file        STC15xxxxxx.h
   ...
   @author	liaokeyre
   @date   	2014.08.10
   @mail   	liaokeyr@foxmail.com
   @qq   	120774639
   @note	Copyright:(c) 2014 , All Rights Reserved.
*/
/*----------------------------------------------------------------------------*/
#ifndef __STC15xxxxxx_H_
...
//sfr IE2       = 0xAF;		-    ET4   ET3  ES4  ES3  ET2  ESPI  ES2   0000,0000B	//Auxiliary Interrupt   
#define		UART2_INT_ENABLE()		    IE2 |=  1	         //允许串口2中断
#define		UART2_INT_DISABLE()		    IE2 &= ~1	         //禁止串口2中断
#define		SPI_INT_ENABLE()	 	    IE2 |=  (1<<1)	     //允许SPI中断
#define		SPI_INT_DISABLE()		    IE2 &= ~(1<<1)	     //禁止SPI中断
#define 	Timer2_InterruptEnable()	IE2 |=  (1<<2)	     //允许Timer2中断.
#define 	Timer2_InterruptDisable()	IE2 &= ~(1<<2)	     //禁止Timer2中断.
#define		UART3_INT_ENABLE()		    IE2 |=  (1<<3)		 //串口3允许中断
#define		UART3_INT_DISABLE()		    IE2 &= ~(1<<3)		 //串口3允许中断
#define		UART4_INT_ENABLE()		    IE2 |=  (1<<4)		 //串口4允许中断
#define		UART4_INT_DISABLE()		    IE2 &= ~(1<<4)		 //串口4允许中断
...
#define 	Timer4_InterruptDisable()	IE2 &= ~(1<<6)	     //禁止Timer3中断.
...
#define		S1_8bit()			SCON  = (SCON & 0x3f) | 0x40	 //UART1  8BIT 模式
...
#define	    S3_BRT_UseTimer3()	S3CON |=  (1<<6)		//串口3波特率定时器选择
#define		S3_8bit()			S3CON &= ~(1<<7)		//串口3模式0，8位UART
#define		S3_9bit()			S3CON |=  (1<<7)		//串口3模式1，9位UART
...
#define	    S4_BRT_UseTimer4()	S4CON |=  (1<<6)		//串口3波特率定时器选择
#define		S4_8bit()			S4CON &= ~(1<<7)		//串口3模式0，8位UART
#define		S4_9bit()			S4CON |=  (1<<7)		//串口3模式1，9位UART
#define		S4_RX_Enable()		S4CON |=  (1<<4)		//允许串3接收
#define		S4_RX_Disable()		S4CON &= ~(1<<4)		//禁止串3接收
...
#define 	ExternalRAM_enable()		AUXR |=  (1<<1)	//允许外部XRAM，禁止使用内部1024RAM
#define 	InternalRAM_enable()		AUXR &= ~(1<<1)	//禁止外部XRAM，允许使用内部1024RAM
#define 	S1_BRT_UseTimer2()	        AUXR |=  1		//选择定时器2作为串口1波特率发生器
#define 	S1_BRT_UseTimer1()	        AUXR &= ~1		//选择定时器1作为串口1波特率发生器
//						   7     6       5      4     3      2      1      0    Reset Value
//sfr T4T3M  = 0xD1;	  T4R  T4_C/T  T4X12  T4CLKO T3R  T3_C/T  T2X12  T3CLKO 0000,0000	//Auxiliary Register 

#define 	Timer4_Run()	 			T4T3M |=  (1<<7)	//允许定时器3计数
#define 	Timer4_Stop()	 			T4T3M &= ~(1<<7)	//禁止定时器3计数
#define		Timer4_AsCounterP05()		T4T3M |=  (1<<6)	//时器3用做计数器
#define		Timer4_AsTimer()			T4T3M &= ~(1<<6)	//时器3用做定时器
#define 	Timer4_1T()					T4T3M |=  (1<<5)	//Timer3 clodk = fo
#define 	Timer4_12T()				T4T3M &= ~(1<<5)	//Timer3 clodk = fo/12	12分频,	default
#define		Timer4_ClkOutEnableP06()	T4T3M |=  (1<<4)	//允许时器4时钟输出
#define		Timer4_ClkOutDisableP06()	T4T3M &= ~(1<<4)	//禁止时器4时钟输出
#define 	Timer3_Run()	 			T4T3M |=  (1<<3)	//允许定时器3计数
#define 	Timer3_Stop()	 			T4T3M &= ~(1<<3)	//禁止定时器3计数
#define		Timer3_AsCounterP05()		T4T3M |=  (1<<2)	//时器3用做计数器
#define		Timer3_AsTimer()			T4T3M &= ~(1<<2)	//时器3用做定时器
#define 	Timer3_1T()					T4T3M |=  (1<<1)	//Timer3 clodk = fo
#define 	Timer3_12T()				T4T3M &= ~(1<<1)	//Timer3 clodk = fo/12	12分频,	default
#define		Timer3_ClkOutEnableP04()	T4T3M |=   1	    //允许时器3时钟输出
#define		Timer3_ClkOutDisableP04()	T4T3M &= ~ 1	    //禁止时器3时钟输出

//sfr INT_CLKO （AUXR2） = 0x8F;	//附加的 SFR WAKE_CLKO (地址：0x8F)
/*
    7   6    5    4   3     2        1       0         Reset Value
...
#define		T1_pulseP35_enable()		AUXR2 |=  (1<<1)	//允许 T1 溢出脉冲在T1(P3.4)脚输出，Fck1 = 1/2 T1 溢出率，T1可以1T或12T。
#define		T1_pulseP35_disable()		AUXR2 &= ~(1<<1)
#define		T2_pulseP30_enable()		AUXR2 |=  (1<<2)	//允许 T2 溢出脉冲在T1(P3.0)脚输出，Fck2 = 1/2 T2 溢出率，T2可以1T或12T。
#define		T2_pulseP30_disable()		AUXR2 &= ~(1<<2)
...
#define     Ex2_int_Enable()			AUXR2 |=  (1<<4)	 //允许外部中断2中断
#define     Ex2_int_Disable()			AUXR2 &= ~(1<<4)	 //禁止外部中断2中断
#define     Ex3_int_Enable()			AUXR2 |=  (1<<5)	 //允许外部中断3中断
#define     Ex3_int_Disable()			AUXR2 &= ~(1<<5)	 //禁止外部中断3中断
#define     Ex4_int_Enable()			AUXR2 |=  (1<<6)	 //允许外部中断4中断
#define     Ex4_int_Disable()			AUXR2 &= ~(1<<6)	 //禁止外部中断4中断
